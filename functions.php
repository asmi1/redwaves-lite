<?php
	...
	*/
	...
	function redwaves_scripts() {
		wp_enqueue_style( 'redwaves-lite-stylesheet', get_stylesheet_uri() );
        ...
        wp_enqueue_style( 'redwaves-lite-roboto-googlefont', 'https://fonts.googleapis.com/css?family=Roboto:400italic,300,700,400', false, null, 'all' );
		...
		// Load Slidebars JS
		wp_enqueue_script( 'redwaves-lite-slidebars', get_template_directory_uri() . '/js/slidebars.min.js', array( 'jquery' ) );

		// Load Slidebars CSS
        wp_enqueue_style( 'redwaves-lite-slidebars', get_template_directory_uri().'/css/slidebars.min.css', false, null, 'all' );

        // Loading jquery.pin.min.js only if the "sticky_menu" option is enabled
		$sticky_menu = get_theme_mod( 'sticky_menu', 1 );
		if ( $sticky_menu == 1 ) {
			wp_enqueue_script( 'redwaves-lite-jquery-pin',  get_template_directory_uri() . '/js/jquery.pin.js', array( 'jquery' ), null, true );
		}
	}
	...
	if ( ! function_exists( 'redwaves_related_posts' ) ) {
		...
		$related_posts_excerpt = get_theme_mod( 'related_posts_excerpt', '10' );
			if ( $related_posts == 1 && intval( $related_posts_number ) > 0 && intval( $related_posts_number ) <= 6 ) {
	...
	/*-----------------------------------------------------------------------------------*/
	function redwaves_prefix_upsell_notice() {
		wp_enqueue_script( 'prefix-customizer-upsell', get_template_directory_uri() . '/js/upsell.js', array(), '1.0.0', true );
		wp_localize_script( 'prefix-customizer-upsell', 'prefixL10n', array( 'prefixURL'	=> esc_url( '//themient.com/themes/redwaves' ), 'prefixLabel'	=> __( 'Upgrade', 'redwaves-lite' ), ) );
	}
	add_action( 'customize_controls_enqueue_scripts', 'redwaves_prefix_upsell_notice' );
